    ...
    def get_seed_rng(
        ...
        if self.seeding == 'hash':
            ...
            for jj,i in enumerate(input_ids):
                ...
                seed = (seed * self.salt_key + int(i)) % self.hash_size
                #print(jj,i,seed)
        elif self.seeding == 'additive':
        ...
        # REPLACE IT WITH THIS:
            ...
            # Convert the list to a PyTorch Tensor so math works properly
            tensor_ids = torch.tensor(input_ids, dtype=torch.long)
            seed_tensor = self.hashint(self.salt_key * tensor_ids)
            seed = int(torch.min(seed_tensor))
