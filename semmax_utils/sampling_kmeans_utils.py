...
def kmeans_reject_completion(
    ...
    actual_max_trials = max_trials if max_trials is not None else MAX_TRIALS
    ...
    while True:
        ...
        if (accepted_text == None and current_trials < actual_max_trials):
            continue
        else:
        ...
        if (curr_cluster_id in mask) or current_trials >= actual_max_trials:
            # if current_trials >= MAX_TRIALS:
            if current_trials >= actual_max_trials:
                # print(
                #     f'WARNING: desired semantic signature can\'t be sampled after max_trials {MAX_TRIALS}', flush=True)
                print(f'WARNING: desired semantic signature can\'t be sampled after max_trials {actual_max_trials}', flush=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_path',type=str,default="watermark/k_semstamp/centroids/booksum-cluster_8_centers.pt",help='Path where cluster centers will be saved')
