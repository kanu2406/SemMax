...
# --------------------------------------------------------------------------- #
# Detectability + cost (from saved detect dicts / gen times)
# --------------------------------------------------------------------------- #
...
def tpr_at_fpr(pos, neg, target):
    # if not pos or not neg:
    #     return float("nan")
    # fpr, tpr, _ = roc_curve([1] * len(pos) + [0] * len(neg), list(pos) + list(neg))
    # ok = fpr <= target
    # return float(tpr[ok].max()) if ok.any() else 0.0
    if not pos or not neg:
        return float("nan")
    tpr_target = DynamicThresholdSuccessRateCalculator(rule="target_fpr", target_fpr=target, reverse=False).calculate(
        watermarked_result=[float(v) for v in pos], non_watermarked_result=[float(v) for v in neg])

    return tpr_target.get("TPR", 0.0)



# --------------------------------------------------------------------------- #
# Quality (MarkLLM analyzers on saved texts, cached)
# --------------------------------------------------------------------------- #
...
def main():
    ...
    for n in sorted(file_by_n):
        ...
        rows.append({
            ...
            "AUROC": auroc(pos, neg),
            "TPR@FPR=1%": tpr_at_fpr(pos, neg, 0.01),
            "TPR@FPR=5%": tpr_at_fpr(pos, neg, 0.05),
