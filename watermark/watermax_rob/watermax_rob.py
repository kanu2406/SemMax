...
from watermark.watermax.wm import NewRobustWmSentenceGenerator, SecureGaussianSentenceWm
...
class RobGaussConfig(BaseConfig):
...
class RobGauss(BaseWatermark):
    """Top-level class for the Gaussian n-gram watermark."""

    def __init__(self, algorithm_config: "str | RobGaussConfig",
                 transformers_config: TransformersConfig | None = None,
                 *args, **kwargs) -> None:
        if isinstance(algorithm_config, str):
            self.config = RobGaussConfig(algorithm_config, transformers_config)
        elif isinstance(algorithm_config, RobGaussConfig):
            self.config = algorithm_config
        else:
            raise TypeError("algorithm_config must be either a path string or a RobGaussConfig instance")
        ...
        self.detector = SecureGaussianSentenceWm(
            tokenizer=c.generation_tokenizer,
            ngram=c.ngram,
            seed=c.seed,
            seeding=c.seeding,
